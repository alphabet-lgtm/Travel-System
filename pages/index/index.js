...
Page({
  data: {
    ...
    choosex:0,
    choosey:0,
    rchoose:app.globalData.rchoose,
    rlocal:app.globalData.rlocal,
    localnewsList:[],
  ...
  addressChoose: function (e) {
    ...
    wx.chooseLocation({
      success: function (res) {
        that.setData({
          address: res.name,
          choosex: res.longitude*100000, //经度
          choosey: res.latitude*100000,//纬度
        })
        console.log(that.data.choosex);
      },
      fail: function (e) {
      },
      complete: function (e) {
        that.AcquireChooseLocationNews();
      }
    })
  },
  //返回当前位置
  addressrefresh:function(){
    //获取当前位置
    var that = this;
    wx.getLocation({
      type: 'wgs84',
      success: function (res) {
        ...
        var longitude = res.longitude
        that.setData({
          localx: latitude * 100000,
          localy: longitude * 100000,
          islocated:0,
          address:'当前位置'
        })
      },
  ...
  //获取选择位置的消息
  AcquireChooseLocationNews: function(){
    var that = this;
    that.setData({
      newsList:[]
    })
    ...
    var choosex = that.data.choosex;
    var choosey = that.data.choosey;
    var rchoose = that.data.rchoose;
    var News = Bmob.Object.extend("news");
    var query = new Bmob.Query(News);
    var kind = that.data.kind;
    query.lessThan('x',choosex + rchoose);
    query.greaterThan('x', choosex - rchoose);
    query.lessThan('y', choosey + rchoose);
    query.greaterThan('y', choosey - rchoose);
    if (kind != '全部') {
      ...
      success: function (results) {
        ...
        // 循环处理查询到的数据
          for (var i = 0; i < results.length; i++) {
            ...
            console.log(object);
            var news = object.get('news');
            var x = object.get('x');
            var y = object.get('y');
            var _url;
            var jsonA;
            var picture = results[i].get("picture");
            console.log(picture)
            if (picture) {
              _url = results[i].get("picture")._url;
              jsonA = {
                "picture": _url,
                "text": news
              }
              ...
              console.log(object.id + ' - ' + object.get('news'));
  ...
  AcquireLocalnews:function(){
    ...
    var molist = new Array();
    console.log('newsList' + that.data.newsList);
    var localx = that.data.localx;
    var localy = that.data.localy;
    var rchoose = that.data.rlocal;
    var kind = that.data.kind;
    var News = Bmob.Object.extend("news");
    var query = new Bmob.Query(News);
    query.lessThan('x', localx + rchoose);
    query.greaterThan('x', localx - rchoose);
    query.lessThan('y', localy + rchoose);
    query.greaterThan('y', localy - rchoose);
    if(kind != '全部')
    {
      query.equalTo("kind", kind);
    }
      ...
      success: function (results) {
        ...
        for (var i = 0; i < results.length; i++) {
          ...
          if (picture) {
            ...
            molist.push(jsonA);
            that.setData({
              localnewsList: molist
            })
            console.log(object.id + ' - ' + object.get('news'));
            console.log(that.data.localnewsList);
            console.log(that.data.localnewsList.length);
          console.log(object.id + ' - ' + object.get('title'));
        }
      },
      error: function (error) {
...
function getTypeName(acttype) {
  ...
  return acttypeName;
    ...
    function createMap(){
      ...
      map.centerAndZoom(new BMap.Point(118.878261,32.084754),15);
    }
